...
└── anno-names.txt                 <- newline-separated cell-type names
```
...
Runs seven sub-steps sequentially for each sample:
...
| 6 | `prepare_inference_new_sample.py` | Converts `single_super_emb.h5ad` → `embeddings-hist.pickle` |
| 7 | `prepare_visium_inputs.py` | Reformats Visium spatial/RCTD data → `locs.tsv`, `radius.txt`, `anno-names.txt`, etc. (skipped for non-Visium) |
...
| `preprocess` | `pixel_size_raw` | `null` | Manual raw MPP override (`null` = auto-detect from image metadata) |

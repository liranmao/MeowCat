...
def harmony_cuML(adata, args):
    ...
    # PCA
    t0 = time()
    X_cp = cp.asarray(adata.X.astype(np.float32))
    ...
    harmony_object = hm.run_harmony(adata.obsm['X_pca'], adata.obs, batch_key)
...
def harmony_cpu(adata,args):
    ...
    harmony_object = hm.run_harmony(adata.obsm['X_pca'], adata.obs, "sample")
    # Store Harmony embeddings
    adata.obsm['X_harmony'] = harmony_object.Z_corr.T

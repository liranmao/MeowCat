...
/project/KidneyHE/01_meowcat_test/03_visium_xenium_single/input/
  VIS_S1/
    ...
    filtered_feature_bc_matrix/
    spatial/
    anno-names.txt   <- must match XEN_S1/anno-names.txt
    radius.txt
  XEN_S1/
    he_raw.tif
    adata_cellbin_HistoSweep.h5ad
    XEN_S1_cell_type_anno.csv
    anno-names.txt   <- must match VIS_S1/anno-names.txt
```

> Both samples must share the **same cell-type vocabulary** (same `anno-names.txt`).
...
**Domain ID assignment (auto):** folder names are sorted alphabetically and assigned IDs 0, 1, 2, 3 → `VIS_S1=0`, `VIS_S2=1`, `XEN_S1=2`, `XEN_S2=3`. To override this, create a TSV:
```
VIS_S1	cohort_A
VIS_S2	cohort_A
XEN_S1	cohort_B
